...
if(isset($_POST['sub']))
{
   $date=date('Y-m-d');
    ...
    $sel=mysql_query("SELECT onme,lat,lon,addr,con,lid, SQRT( POW(69.1 * (lat - $la), 2) + POW(69.1 * ($lo - lon) * COS(lat / 57.3), 2)) AS distance FROM local  HAVING distance < 60 ORDER BY distance");
    $i=0;
    if(mysql_affected_rows()>0)
{
    ...
    while($r=mysql_fetch_row($sel))
    {
     ...
     <br/>
                                        <a href="home_local.php?cid=<?php echo $r[5] ?>" class="btn btn-danger">More</a>
