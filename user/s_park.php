...
if(isset($_POST['sub']))
{
   $date=date('Y-m-d');
    ...
    $sel=mysql_query("SELECT id,onme,lat,lon,addr,con,lid, SQRT( POW(69.1 * (lat - $la), 2) + POW(69.1 * ($lo - lon) * COS(lat / 57.3), 2)) AS distance FROM org_data where ot='4' and st='1' HAVING distance < 60 ORDER BY distance");
    $i=0;
